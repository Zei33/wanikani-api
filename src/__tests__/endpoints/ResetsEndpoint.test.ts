import { ResetsEndpoint } from '../../classes/endpoints/ResetsEndpoint.js';
...
describe('ResetsEndpoint', () => {
	...
	beforeEach(() => {
		...
		endpoint = new ResetsEndpoint(request);
	});

	describe('getAll', () => {
		it('should make a request to get all resets', async () => {
			const mockData = [{
				id: 1,
				original_level: 20,
				target_level: 15,
	...
	describe('get', () => {
		it('should make a request to get a specific reset', async () => {
			const mockData = {
				id: 1,
				original_level: 20,
				target_level: 15,
		...
		it('should handle API errors', async () => {
			mockFetch.mockImplementationOnce(async () => ({
				...
				ok: false,
				headers: new Map([['Content-Type', 'application/json']]) as unknown as Headers,
				json: async () => ({ error: 'Reset not found' })
			}));
