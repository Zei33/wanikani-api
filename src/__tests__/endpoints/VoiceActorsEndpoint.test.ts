import { VoiceActorsEndpoint } from '../../classes/endpoints/VoiceActorsEndpoint.js';
...
describe('VoiceActorsEndpoint', () => {
	...
	let endpoint: VoiceActorsEndpoint;
	...
	beforeEach(() => {
		...
		endpoint = new VoiceActorsEndpoint(request);
	});

	describe('getAll', () => {
		it('should make a request to get all voice actors', async () => {
			const mockData = [{
				id: 1,
				description: 'A professional voice actor',
				gender: 'female',
				name: 'Kyoko Hikami',
	...
	describe('get', () => {
		it('should make a request to get a specific voice actor', async () => {
			const mockData = {
				id: 1,
				description: 'A professional voice actor',
				gender: 'female',
		...
		it('should handle API errors', async () => {
			mockFetch.mockImplementationOnce(async () => ({
				...
				ok: false,
				headers: new Map([['Content-Type', 'application/json']]) as unknown as Headers,
				json: async () => ({ error: 'Voice actor not found' })
			}));
